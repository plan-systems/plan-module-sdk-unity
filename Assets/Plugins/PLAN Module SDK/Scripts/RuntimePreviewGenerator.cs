...
public static class RuntimePreviewGenerator
{
	...
	public static Texture2D GenerateModelPreview( Transform model, int texSz, Bounds bounds) {
		return GenerateModelPreviewWithShader( model, null, null, texSz, bounds);
	}

	public static Texture2D GenerateModelPreviewWithShader( Transform model, Shader shader, string replacementTag, int texSz, Bounds bounds) {
		...
		bool isStatic = false; //IsStatic( model );
		...
		try
		{
			...
			Vector3 previewDir = m_previewDirection;
            ...
            var previewBounds = new Bounds();
            {
                ...
                for( int i = 0; i < renderersList.Count; i++ ) {
                    ...
                    if ( !init ) {
                        ...
                        var ps = previewObject.GetComponent<ParticleSystem>();
                        if (ps != null) {
                            // ps.time = 1.0f; //set value and play straightly
                            // ps.Simulate(1.1f, true, false); //usual api
                            break;
                        }
                    } else {
                ...
                if (ext.sqrMagnitude > .01f) {
                    // // auto-detect the camera axis
                    if ( Mathf.Abs(ext.y) * 5 < Mathf.Abs(ext.x) && 
                         Mathf.Abs(ext.y) * 5 < Mathf.Abs(ext.z) ) {

                        previewDir = new Vector3(0, 0, -1);
                    }
                    previewBounds.extents = ext;
                    // if (previewBounds.center.sqrMagnitude < .1) {
                    //    previewBounds.center = PREVIEW_POSITION;
                    // }
                }
            }
			...
			Vector3 boundsSize = 2.055f * boundsExtents;
			...
			RenderTexture renderTex = RenderTexture.GetTemporary( texSz, texSz, 16 );
			...
			result = new Texture2D( texSz, texSz, m_backgroundColor.a < 1f ? TextureFormat.RGBA32 : TextureFormat.RGB24, false );
			result.ReadPixels( new Rect( 0, 0, texSz, texSz ), 0, 0, false );
